...
@Entry(
    "avoid_player_activity",
    "Keep a safe distance from players",
    Colors.GREEN,
    "mdi:account-arrow-left"
)
class AvoidPlayerActivityEntry(
    override val id: String = "",
    override val name: String = "",
    @Help("Minimum distance to keep from players")
    val safeDistance: Double = 5.0,
    @Help("Maximum range to consider players")
    ...
    override fun create(
        ...
        return AvoidPlayerActivity(safeDistance, radius, detectionRadius, currentLocation, criteria)
    }
}

class AvoidPlayerActivity(
    ...
    override fun tick(context: ActivityContext): TickResult {
        ...
        val nearbyPlayers = context.viewers.filter {
            val distFromOrigin = origin.distanceSquared(it.location) ?: Double.MAX_VALUE
        ...
        val target = nearbyPlayers
            .filter {
                (currentPosition.distanceSquared(it.location) ?: Double.MAX_VALUE) <= radiusSquared
            }
            .minByOrNull { it.location.distanceSquared(npcLoc) }
            ?: return TickResult.IGNORED

        val dir = npcLoc.toVector().subtract(target.location.toVector())
        ...
        var move = dir.normalize().multiply(safeDistance - distance)
        val maxStep = context.entityState.speed.toDouble()
        if (move.lengthSquared() > maxStep * maxStep) {
        ...
        currentPosition = currentPosition.add(move.x, move.y, move.z)
        avoidedPlayers.clear()
        avoidedPlayers.add(target.uniqueId)
        return TickResult.CONSUMED
    }
