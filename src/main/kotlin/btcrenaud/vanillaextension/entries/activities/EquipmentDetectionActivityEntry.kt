...
@Entry(
    "equipment_detection_activity",
    "React based on player equipment",
    Colors.GREEN,
    "mdi:shield-sword"
)
class EquipmentDetectionActivityEntry(
    override val id: String = "",
    override val name: String = "",
    @Help("Material to detect on the player")
    ...
    override fun create(
        ...
        return EquipmentDetectionActivity(material, radius, detectionRadius, currentLocation, triggers, criteria)
    }
}

class EquipmentDetectionActivity(
    ...
    val equippedPlayers: MutableSet<UUID> = mutableSetOf()
    ...
    override fun tick(context: ActivityContext): TickResult {
        val players = context.viewers.filter {
            val fromOrigin = origin.distanceSquared(it.location) ?: Double.MAX_VALUE
        ...
        val currentlyEquipped = mutableSetOf<UUID>()
        players.forEach { player ->
            val dist = currentPosition.distanceSquared(player.location) ?: return@forEach
            if (dist > radiusSquared) return@forEach
            val inv = player.inventory
            val hasItem = inv.itemInMainHand.type == material ||
                    inv.itemInOffHand.type == material ||
                    inv.armorContents.any { it?.type == material }
            if (hasItem) {
                currentlyEquipped.add(player.uniqueId)
                if (equippedPlayers.add(player.uniqueId)) {
                    triggers.triggerEntriesFor(player) { }
                }
            }
        }
        
        equippedPlayers.removeIf { it !in currentlyEquipped }
        hasEquipped = currentlyEquipped.isNotEmpty()
        return TickResult.IGNORED
    }
