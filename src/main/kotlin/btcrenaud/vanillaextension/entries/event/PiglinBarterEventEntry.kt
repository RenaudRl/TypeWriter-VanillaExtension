...
fun onPiglinBarter(event: EntityDropItemEvent, query: Query<PiglinBarterEventEntry>) {
    ...
    // Get the piglin location
    val piglinLocation = event.entity.location
    
    // Find nearby players who might have triggered the barter
    val nearbyPlayers = piglinLocation.world?.players?.filter { 
        it.location.distanceSquared(piglinLocation) <= 100.0 
    } ?: emptyList()
