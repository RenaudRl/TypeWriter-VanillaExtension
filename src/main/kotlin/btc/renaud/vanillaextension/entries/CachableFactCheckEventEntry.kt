...
@Entry("cachable_fact_check_event", "An event that triggers when a cachable fact matches a value", Colors.YELLOW, "ph:check-circle-bold")
class CachableFactCheckEventEntry(
    ...
    @Help("The cachable fact to check.")
    ...
    override suspend fun display(): AudienceFilter {
        return CachableFactCheckEventDisplay(ref())
    }
}

private class CachableFactCheckEventDisplay(private val ref: Ref<CachableFactCheckEventEntry>) : AudienceFilter(ref) {
    ...
    private fun startTimer() {
        ...
        if (entry.checkIntervalSeconds <= 0) return

        timerJob = Dispatchers.UntickedAsync.launch {
            while (true) {
                delay(entry.checkIntervalSeconds * 1000L) // Convert to milliseconds
    ...
    private fun matchesFactFor(player: Player, entry: CachableFactCheckEventEntry): Boolean {
