...
import com.typewritermc.entity.entries.activity.TargetLocationActivity
import com.typewritermc.roadnetwork.RoadNetworkEntry
import com.typewritermc.engine.paper.entry.Modifier

@Entry(
    "target_location_visibility_detection_activity",
    "Navigate to a location while detecting players",
    Colors.BLUE,
    "mdi:map-marker-path"
)
class TargetLocationVisibilityDetectionActivityEntry(
    ...
    val idleActivity: Ref<GenericEntityActivityEntry> = emptyRef(),
    ...
    override fun create(
        ...
        return TargetLocationVisibilityDetectionActivity(targetActivity, vision)
    }
}

class TargetLocationVisibilityDetectionActivity(
    val targetActivity: TargetLocationActivity,
    val vision: VisibilityDetectionActivity
) : EntityActivity<ActivityContext> {

    override var currentPosition: PositionProperty
        get() = targetActivity.currentPosition
        set(_) {}
    ...
    override fun initialize(context: ActivityContext) {
        targetActivity.initialize(context)
        vision.initialize(context)
    }

    override fun tick(context: ActivityContext): TickResult {
        val targetResult = targetActivity.tick(context)
        
        vision.currentPosition = targetActivity.currentPosition
        val visionResult = vision.tick(context)

        return if (targetResult == TickResult.CONSUMED || visionResult == TickResult.CONSUMED) {
            TickResult.CONSUMED
        } else {
            TickResult.IGNORED
        }
